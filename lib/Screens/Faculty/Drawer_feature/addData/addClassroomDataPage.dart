...
class AddClassroomDataPage extends StatefulWidget {
  @override
  _AddClassroomDataPageState createState() => _AddClassroomDataPageState();
}

class _AddClassroomDataPageState extends State<AddClassroomDataPage> {
  ...
  Future<void> _addFacultyData() async {
    ...
    try {
      await _firestore.collection('classroom').doc(_name).set({
        'classroom_no': _name,
      ...
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Classroom data added successfully')));
    } catch (e) {
      ...
      body: SingleChildScrollView(
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              TextFormField(
                decoration: InputDecoration(labelText: 'Classroom no'),
                onChanged: (value) {
                  setState(() {
                    _name = value;
                  });
