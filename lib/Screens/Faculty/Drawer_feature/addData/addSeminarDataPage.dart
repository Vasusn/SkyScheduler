...
class AddSeminarDataPage extends StatefulWidget {
  @override
  _AddSeminarDataPageState createState() => _AddSeminarDataPageState();
}

class _AddSeminarDataPageState extends State<AddSeminarDataPage> {
  ...
  Future<void> _addFacultyData() async {
    ...
    try {
      await _firestore.collection('Seminar').doc(_name).set({
        'seminar_no': _name,
      ...
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Seminar data added successfully')));
    } catch (e) {
      ...
      body: SingleChildScrollView(
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              TextFormField(
                decoration: InputDecoration(labelText: 'Seminar no'),
                onChanged: (value) {
                  setState(() {
                    _name = value;
                  });
